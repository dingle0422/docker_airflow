...
import sys
sys.path.append(r"/usr/local/airflow/app_folder/modules")
...
def getconfig(section,option):
    ...
    config.read(r"/usr/local/airflow/app_folder/config.ini")
...
def exportdata():
    ...
    #df0 = connector.getdata()
     ...
     # 本地文件测试用代码
    df0 = pd.read_csv(r'/usr/local/airflow/app_folder/data/implant_data.gz', encoding = 'utf-8')
...
def rele_diag():
    relevant_diag = set(pd.read_csv(r'/usr/local/airflow/app_folder/data/implant_diags.csv', index_col=0)['诊断名称'].unique())
...
def multimodel(data, model_names):
    ...
    '''
    name = locals()
    SVC = joblib.load(r"/usr/local/airflow/app_folder/models/SVC_SMOTE.m")
    XGB = joblib.load(r"/usr/local/airflow/app_folder/models/XGB_OUT_oversample_20201210_131900.m")
    LR = joblib.load(r"/usr/local/airflow/app_folder/models/LR_20201207_30.m")
